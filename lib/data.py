...
from .util import Part, TaskType, get_logger
...
class Dataset(Generic[T]):
    ...
    @classmethod
    def from_dir(cls, path: Union[Path, str]) -> 'Dataset[np.ndarray]':
        ...
        return Dataset(
            {
                key: {
                    part.value: np.load(path / f'{key}_{part.value}.npy')
                    for part in Part
                }
    ...
    def merge_num_bin(self) -> 'Dataset':
...
def transform_cat(X: NumpyDict, policy: CatPolicy) -> NumpyDict:
    ...
    # >>> encode
    if policy == policy.ORDINAL:
        return X
    elif policy == policy.ONE_HOT:
    ...
    *,
    ...
    if cache:
        ...
        if cache_path.exists():
            ...
            if args == cached_args:
                get_logger().debug(f"Using cached features: {cache_path.name}")
                return cached_value
            else:
